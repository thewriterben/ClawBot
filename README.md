...
**Status:** pre-alpha. Four schemas, twenty-eight ADRs, seven scripts, an MCP surface, a zero-dependency Rust binding, **191 Python tests and 37 Rust tests**, all run by CI, and a knowledge base whose robotics half is no longer empty. `data/` holds four real records, and not one of them can size a mechanism. The Dynamixel XM430-W350 comes from a vendor that **names** the stall/continuous distinction and then publishes only stall. The TowerPro MG90S comes from one that does not name it at all — and whose two own documents disagree about the servo's body length by 7 mm. The third is the servo actually on the bench: an unbranded clone with **no torque figure of any kind**, because no datasheet describes it (ADR-0024), which is a stronger refusal than either — not that a vendor published only stall, but that there is no vendor. The first robot record is a two-DOF pan-tilt built on that MG90S, and it was **written before the mechanism was built** — deliberately. Its structure and actuator wiring are real; every dimension is a `TODO(source)` placeholder and both joints carry `limits: null`. So `kinematics reach` answers `incomplete` and names joint `pan`, `affordance` binds on reach and refuses, and URDF export refuses both joints by name. `validate.py` is therefore the build checklist rather than a post-hoc audit — it says what is still owed instead of letting a missing number pass as a zero. A described-from-memory arm is exactly the invented data the schema exists to refuse; a declared-and-blocked one is not the same thing.

...
- [x] **CI** — validator, both test suites, `cargo test`, and the binding staleness gate, on every push and PR. Nothing had been running 180 tests. Two guards borrowed from OpenDesignCore's citations job, which fails rather than passes a check that checked nothing: the build refuses to proceed if OpenBuildCore is not checked out beside this repo (the seam tests would skip) or if the MCP SDK is missing (the ADR-0016 guards would skip). Plus `citations.yml`, weekly, because every robotics source here is cited by URL rather than copied, so link rot is the whole exposure (2026-08-22)
...
- [x] **A Rust binding**, following OpenPartsCore's codegen discipline — generated, committed, `--check` gated, zero dependencies. And it does something the JSON cannot: **it makes the refusals compile errors** rather than conventions. `Radians`/`Degrees` are distinct so the Oh-Ben-Claw seam fails to compile; `StallTorque` has no conversion to `ContinuousTorque`; unknown stays `Option` with no unwrapping accessor. Three `compile_fail` doctests assert the guarantees that cannot be tested at runtime (ADR-0017) (2026-08-22)
...
- [ ] Resolve the degrees/radians seam with Oh-Ben-Claw. `ServoAngle` is degrees; this repo and REP-103 are radians. **This side is now complete:** ADR-0017's newtypes make it uncatchable-by-accident, and ADR-0020 added the field it actually lives on — `Channel::actuator_angle` applies inversion and zero offset and returns radians, so a consumer writes `Degrees::from(...)` at its own edge. The other half is Oh-Ben-Claw consuming the binding, which is upstream's call — the same unfinished half OpenPartsCore has been waiting on.
...
- [x] **A position on PD-5 legality gating** (ADR-0019). ClawBot carries the author's declaration and refuses to make one on their behalf. Every derivation runs regardless — computing kinematics is not fabricating. The refusal is at the output boundary: `manifest.py --as-project` declines for an undeclared record, and for the four categories BINGO refuses network-wide, because that document would have no valid destination. ClawBot never infers a category from geometry (2026-08-22)
...
- **A derivation over starting and backdriving torque.** The fields exist as of ADR-0021 — ranges, because the variation is unit to unit and spans an order of magnitude — and they feed nothing. What is missing is not data but a **source for the relationship**: that a load below the backdriving minimum is held with no actuator torque is a physical claim nothing read here supports, and its failure mode is a joint that lets go.
- **Three recorded-but-unused fields is a smell worth watching.** `measured_efficiency`, `harness.service_loop_mm`, and now the two torque ranges are all data nothing computes with. Each was added because a *source* arrived rather than because a feature was wanted, and each has a written condition for becoming useful. A fourth without one is the point to ask whether the schema is collecting rather than modelling.

...
- [ ] Resolve the degrees/radians seam with Oh-Ben-Claw. `ServoAngle` is degrees; this repo and REP-103 are radians. **Half done:** ADR-0017's `Radians`/`Degrees` newtypes mean a consumer of the binding cannot get it wrong silently. The other half is Oh-Ben-Claw actually consuming the binding, which is upstream's call — the same unfinished half OpenPartsCore has been waiting on.

...
`check_claims`), an MCP
surface and a zero-dependency Rust binding. 191 Python tests, 37 Rust tests, all run by CI. `data/` holds three
actuator records and one robot record — a two-DOF pan-tilt, written BEFORE the build so
